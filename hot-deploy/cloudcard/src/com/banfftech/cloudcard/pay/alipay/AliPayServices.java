...
import com.banfftech.cloudcard.pay.alipay.vo.OrderInfo;

public class AliPayServices {

	public static final String module = AliPayServices.class.getName();

	public static void getApiConfig(String app_id, String alipay_public_key, String charset, String private_key,
			String service_url, String sign_type) {
		AliPayApiConfig aliPayApiConfig = AliPayApiConfig.New().setAppId(app_id).setAlipayPublicKey(alipay_public_key)
				.setCharset(charset).setPrivateKey(private_key).setServiceUrl(service_url).setSignType(sign_type)
				.build();
	...
	public static Map<String, Object> prepayOrder(DispatchContext dctx, Map<String, Object> context) {
		...
		String rsaPrivate = EntityUtilProperties.getPropertyValue("cloudcard.properties", "aliPay.rsa_private",
				delegator);
		...
		orderInfo.setOutTradeNo(receiptPaymentId);
		orderInfo.setSellerId(seller);
				...
				if (UtilValidate.isNotEmpty(noticeMap)) {
					...
					if ("PMNT_RECEIVED".equals(payment.getString("statusId"))) {
						...
						if ("TRADE_SUCCESS".equals(tradeStatus) || "TRADE_FINISHED".equals(tradeStatus)) {
							...
							String kupangPartner = EntityUtilProperties.getPropertyValue("cloudcard",
									"aliPay.kupangAppID", delegator);
							String kuPangPublicKey = EntityUtilProperties.getPropertyValue("cloudcard",
									"aliPay.kupangPublicKey", delegator);
							String kupangRsaPrivate = EntityUtilProperties.getPropertyValue("cloudcard.properties",
							...
							if (!ServiceUtil.isSuccess(rechargeCloudCardDepositOutMap)) {
								...
								getApiConfig(kupangPartner, kuPangPublicKey, "utf-8", kupangRsaPrivate, service_url,
										signType);
								...
								if (isSuccess) {
									...
									try {
										setPaymentStatusOutMap = dispatcher.runSync("setPaymentStatus",
												UtilMisc.toMap("userLogin", systemUserLogin, "locale", null,
														"paymentId", paymentId, "statusId", "PMNT_CONFIRMED"));
