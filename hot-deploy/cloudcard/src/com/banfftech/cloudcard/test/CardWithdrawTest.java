...
public class CardWithdrawTest extends CloudCardServicesTest {
	 ...
	 */
	public void testReceiptByCardCode() throws GenericServiceException, GenericEntityException{
		...
		resp = callReceiptByCardCode(CUSTOMER_1_TEL, STORE_ID_1, cardCode, paymentAmount);
		assertFalse("Service 'receiptByCardCode' SHOULD NOT result success while the user is not the manager of a store", ServiceUtil.isSuccess(resp));

		// 测试2 收款金额大于卡余额，不应该成功
		resp = callReceiptByCardCode(STORE_1_USER,  STORE_ID_1, cardCode, cardBalance.add(paymentAmount));
		assertFalse("Service 'receiptByCardCode' SHOULD NOT result success when the card balance is NOT enough", ServiceUtil.isSuccess(resp));

		// 测试3 收款金额为0，不应该成功
		resp = callReceiptByCardCode(STORE_1_USER,  STORE_ID_1, cardCode, ZERO);
		assertFalse("Service 'receiptByCardCode' SHOULD NOT result success when the payment amount is ZERO", ServiceUtil.isSuccess(resp));

		// 测试4 收款金额为负数，不应该成功
		resp = callReceiptByCardCode(STORE_1_USER,  STORE_ID_1, cardCode, paymentAmount.negate());
		assertFalse("Service 'receiptByCardCode' SHOULD NOT result success when the payment amount is less than zero", ServiceUtil.isSuccess(resp));

		// 测试5 正常收款
		resp = callReceiptByCardCode(STORE_1_USER,  STORE_ID_1, cardCode, paymentAmount);
		assertTrue("Service 'receiptByCardCode' SHOULD result success", ServiceUtil.isSuccess(resp));
	 ...
	 * @throws GenericEntityException
	 */
	public void testCustomerWithdraw() throws GenericServiceException, GenericEntityException{
		// 获取用户卡
		Map<String, Object> resp = callMyCloudCards(CUSTOMER_1_TEL, 0, 20);
		assertTrue("Service 'myCloudCards' SHOULD result success", ServiceUtil.isSuccess(resp));
		...
		Map<String,Object> oneCard = null;
		BigDecimal paymentAmount = new BigDecimal(15.23f);
		...
		// 测试1 登录用户不是 拥有卡的持卡人，不应该成功
		resp = callCustomerWithdraw(CUSTOMER_2_TEL, STORE_1_QR_CODE, cardId, paymentAmount);
		assertFalse("Service 'customerWithdraw' SHOULD NOT result success while the user is not owenr of this card", ServiceUtil.isSuccess(resp));

		// 测试2 收款金额大于卡余额，不应该成功callCustomerWithdraw(CUSTOMER_2_TEL, STORE_1_QR_CODE, cardId, paymentAmount);
		resp = callCustomerWithdraw(CUSTOMER_1_TEL, STORE_1_QR_CODE, cardId, cardBalance.add(paymentAmount));
		...
		// 测试3 收款金额为0，不应该成功
		resp = callCustomerWithdraw(CUSTOMER_1_TEL, STORE_1_QR_CODE, cardId, ZERO);
		...
		// 测试4 收款金额为负数，不应该成功
		resp = callCustomerWithdraw(CUSTOMER_1_TEL,  STORE_1_QR_CODE, cardId, paymentAmount.negate());
		...
		// 测试5 正常收款
		resp = callCustomerWithdraw(CUSTOMER_1_TEL,  STORE_1_QR_CODE, cardId, paymentAmount);
