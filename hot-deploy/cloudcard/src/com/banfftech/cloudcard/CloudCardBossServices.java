...
public class CloudCardBossServices {
	...
	public static Map<String, Object> bizCreateApply(DispatchContext dctx, Map<String, Object> context) {
		...
		EntityCondition captchaCondition = EntityCondition.makeCondition(
				...
				EntityCondition.makeCondition("smsType", EntityOperator.EQUALS,
						CloudCardConstant.BIZ_CREATE_STORE_CAPTCHA));
		...
		try {
			...
			return ServiceUtil
					.returnError(UtilProperties.getMessage(resourceError, "CloudCardGetCAPTCHAFailedError", locale));
		}
		...
		try {
			...
			if (null != userByTeleNumber) {
				List<String> organizationPartyIds = CloudCardHelper.getOrganizationPartyId(delegator,
						userByTeleNumber.getString("partyId"));
				if (UtilValidate.isNotEmpty(organizationPartyIds)) {
					...
					return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
							"CloudCardTelHasBeenAssociatedWithAstoreOwner", locale));
				}
			}

			/*
			 * List<GenericValue> oldAnswers =
			 * delegator.findList("SurveyResponseAndAnswer",
			 * EntityCondition.makeCondition(EntityOperator.OR,
			 * EntityCondition.makeCondition(UtilMisc.toMap("surveyQuestionId",
			 * "SQ_CC_S_OWNER_TEL", "textResponse", teleNumber)),
			 * EntityCondition.makeCondition(UtilMisc.toMap("surveyQuestionId",
			 * "SQ_CC_STORE_NAME", "textResponse", storeName))), null,
			 * UtilMisc.toList("-answeredDate"), null, false);
			 *
			 * // 检查是否有状态为 创建 和 已通过 的数据 oldAnswers =
			 * EntityUtil.filterByOr(oldAnswers,
			 * UtilMisc.toList(EntityCondition.makeCondition("statusId",
			 * "SRS_CREATED"), EntityCondition.makeCondition("statusId",
			 * "SRS_ACCEPTED")));
			 *
			 * if (UtilValidate.isNotEmpty(oldAnswers)) {
			 * Debug.logError("teleNumber[" + teleNumber + "] or storeName[" +
			 * storeName + "] has been used to apply for a shop", module);
			 * return ServiceUtil.returnError(UtilProperties.getMessage(
			 * CloudCardConstant.resourceError,
			 * "CloudCardRepeatedApplyForCreateStore", locale)); }
			 */
		} catch (GenericEntityException e) {
			Debug.logError(e.getMessage(), module);
			return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
		...
		try {
			...
			try {
				...
				EntityCondition provinceNameCond = EntityCondition.makeCondition("geoName", EntityOperator.LIKE,
						provinceTemp + "%");
				EntityCondition provinceTypeCond = EntityCondition.makeCondition("geoTypeId", EntityOperator.EQUALS,
						"PROVINCE");
				EntityCondition provinceCond = EntityCondition.makeCondition(EntityOperator.AND, provinceNameCond,
						provinceTypeCond);

				EntityCondition cityNameCond = EntityCondition.makeCondition("geoName", EntityOperator.LIKE,
						cityTemp + "%");
				EntityCondition cityTypeCond = EntityCondition.makeCondition("geoTypeId", EntityOperator.EQUALS,
						"CITY");
				EntityCondition cityCond = EntityCondition.makeCondition(EntityOperator.AND, cityNameCond,
						cityTypeCond);

				EntityCondition countyNameCond = EntityCondition.makeCondition("geoName", EntityOperator.LIKE,
						countyTemp + "%");
				EntityCondition countyTypeCond = EntityCondition.makeCondition("geoTypeId", EntityOperator.EQUALS,
						"COUNTY");
				EntityCondition countyCond = EntityCondition.makeCondition(EntityOperator.AND, countyNameCond,
						countyTypeCond);

				EntityCondition addrCond = EntityCondition.makeCondition(EntityOperator.OR, provinceCond, cityCond,
						countyCond);
				List<GenericValue> geoList = delegator.findList("Geo", addrCond, UtilMisc.toSet("geoId", "geoTypeId"),
						null, null, true);
			...
			String allowCrossStorePay = EntityUtilProperties.getPropertyValue("cloudcard", "store.allowCrossStorePay",
					delegator);
			...
			storeInfoMap.put("allowCrossStorePay", allowCrossStorePay);// 是否允许本店卡跨店付款
																		// Y/N
		...
		/*
		 * // 创建“调查回答”，用于记录开店申请者提交的信息 Map<String, Object>
		 * createSurveyResponseOutMap; try { createSurveyResponseOutMap =
		 * dispatcher.runSync("createSurveyResponse", UtilMisc.toMap("locale",
		 * locale, "surveyId", surveyId, "statusId", statusId, "answers",
		 * UtilMisc.toMap("SQ_CC_STORE_NAME", storeName, "SQ_CC_STORE_ADDR",
		 * storeAddress, "SQ_CC_S_OWNER_TEL", teleNumber, "SQ_CC_S_OWNER_NAME",
		 * personName, "SQ_CC_STORE_DESC", description, "SQ_CC_S_LONGITUDE",
		 * longitude, "SQ_CC_S_LATITUDE", latitude))); } catch
		 * (GenericServiceException e1) { Debug.logError(e1.getMessage(),
		 * module); return
		 * ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.
		 * resourceError, "CloudCardInternalServiceError", locale)); } if
		 * (!ServiceUtil.isSuccess(createSurveyResponseOutMap)) { return
		 * createSurveyResponseOutMap; }
		 */
		...
		// result.put("responseId",
		// createSurveyResponseOutMap.get("surveyResponseId"));
	...
	public static Map<String, Object> bizCreateGroup(DispatchContext dctx, Map<String, Object> context) {
		...
		try {
			...
			// 检查是否已经 建立/加入 圈子
			GenericValue myGroupRelationship = CloudCardHelper.getGroupRelationShipByStoreId(delegator,
					organizationPartyId, true);
			if (UtilValidate.isNotEmpty(myGroupRelationship)) {
				// 已经在圈子里，不能创建新的圈子
				return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
						"CloudCardIsInGroupCanNotCreateAnother", locale));
			}
			...
			Map<String, Object> createPartyRoleOut = dispatcher.runSync("createPartyRole",
					UtilMisc.toMap("locale", locale, "userLogin", systemUser, "partyId", groupId, "roleTypeId",
							CloudCardConstant.STORE_GROUP_ROLE_TYPE_ID));
			...
			Map<String, Object> ensurePartyRoleOut = dispatcher.runSync("ensurePartyRole",
					UtilMisc.toMap("locale", locale, "userLogin", systemUser, "partyId", organizationPartyId,
							"roleTypeId", CloudCardConstant.STORE_GROUP_OWNER_ROLE_TYPE_ID));
			...
			Map<String, Object> relationOutMap = dispatcher.runSync("createPartyRelationship",
					UtilMisc.toMap("locale", locale, "userLogin", systemUser, "partyIdFrom", groupId, "partyIdTo",
							organizationPartyId, "roleTypeIdFrom", CloudCardConstant.STORE_GROUP_ROLE_TYPE_ID,
							"roleTypeIdTo", CloudCardConstant.STORE_GROUP_OWNER_ROLE_TYPE_ID, "partyRelationshipTypeId",
							CloudCardConstant.STORE_GROUP_PARTY_RELATION_SHIP_TYPE_ID));
	...
	public static Map<String, Object> bizSentGroupInvitation(DispatchContext dctx, Map<String, Object> context) {
		...
		try {
			...
			if (null == ourStoreGroup) {
				// 自己不存在圈子
				Debug.logWarning(
						"This store[" + organizationPartyId + "] is not in a group of store, can not invite others",
						module);
				return ServiceUtil.returnError(
						UtilProperties.getMessage(CloudCardConstant.resourceError, "CloudCardIsNotInAgroup", locale));
			}
			boolean isOwner = CloudCardHelper.isStoreGroupOwner(delegator, organizationPartyId, true);
			if (!isOwner) {
				// 自己不是圈主
				Debug.logWarning(
						"This store[" + organizationPartyId + "] is not a store group owner, can not invite others",
						module);
				return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
						"CloudCardIsNotAstoreGroupOwner", locale));
			}
			...
			if (null == userByTeleNumber) {
				// 被邀请用户未注册
				Debug.logWarning(
						"This phone number [" + teleNumber + "] is not registered as a user, can not be invited",
						module);
				return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
						"CloudCardTelIsNotAssociatedWithAstoreOwner", locale));
			}
			List<String> storeIdList = CloudCardHelper.getOrganizationPartyId(delegator,
					userByTeleNumber.getString("partyId"));
			...
			if (storeIdList.contains(organizationPartyId)) {
				// 被邀请用户和自己是同一个店的两个管理员
				Debug.logWarning("This user [" + teleNumber + "]  and you in the same store[" + organizationPartyId
						+ "], can not be invited", module);
				return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
						"CloudCardUserIsInTheSameStore", locale));
			}
			...
			if (null != otherStoreGroup) {
				// 已经加入某个圈子了
				Debug.logWarning("This store[" + storeId + "] is already in a Group, Can Not join another one.",
						module);
				return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
						"CloudCardIsInGroupCanNotJoinAnother", locale));
			}
		...
		try {
			oldPartyInvitations = delegator.findList("PartyInvitation",
					EntityCondition
							.makeCondition(UtilMisc.toMap("partyIdFrom", organizationPartyId, "partyId", storeId)),
		...
		if (UtilValidate.isNotEmpty(oldPartyInvitations)) {
			for (GenericValue gv : oldPartyInvitations) {
				...
				if ("PARTYINV_SENT".equals(oldPartyInvitationStatus) || "".equals(oldPartyInvitationStatus)) {
					Debug.logWarning(
							"There has been an effective invitation[" + gv.getString("partyInvitationId") + "]",
							module);
					return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
							"CloudCardInvitationAlreadyExists", locale));
				}
			}
		}
		...
		try {
			createPartyInvitationOutMap = dispatcher.runSync("createPartyInvitation",
					UtilMisc.toMap("locale", locale, "userLogin", userLogin, "partyId", storeId, "partyIdFrom",
							organizationPartyId, "statusId", "PARTYINV_SENT"));
			...
			Map<String, Object> createPartyInvitationGroupAssocOutMap = dispatcher
					.runSync("createPartyInvitationGroupAssoc", UtilMisc.toMap("locale", locale, "userLogin", userLogin,
							"partyInvitationId", partyInvitationId, "partyIdTo", groupId));
			if (!ServiceUtil.isSuccess(createPartyInvitationGroupAssocOutMap)) {
				return createPartyInvitationGroupAssocOutMap;
			}

			Map<String, Object> createPartyInvitationRoleAssocOutMap = dispatcher
					.runSync("createPartyInvitationRoleAssoc",
							UtilMisc.toMap("locale", locale, "userLogin", userLogin, "partyInvitationId",
									partyInvitationId, "roleTypeId",
									CloudCardConstant.STORE_GROUP_PARTNER_ROLE_TYPE_ID));
	...
	public static Map<String, Object> bizAcceptGroupInvitation(DispatchContext dctx, Map<String, Object> context) {
		...
		try {
			...
			partyInvitation = delegator.findByPrimaryKey("PartyInvitation",
			...
			if (null == partyInvitation) {
				...
				return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
						"CloudCardCanNotFindInvitation", locale));
			}

			// 检查partyInvitationId是否针对本店的
			if (!organizationPartyId.equals(partyInvitation.getString("partyId"))) {
				Debug.logWarning("Illegal data access PartyInvitation[" + organizationPartyId
						+ "] is NOT related to Store[" + organizationPartyId + "]", module);
				return ServiceUtil.returnError(
						UtilProperties.getMessage(CloudCardConstant.resourceError, "CloudCardWasNotInvited", locale));
			}

			// 检查partyInvitation的状态是否为PARTYINV_SENT
			String statusId = partyInvitation.getString("statusId");
			if (!"PARTYINV_SENT".equals(statusId)) {
				Debug.logWarning("partyInvitation statusId is error, expected PARTYINV_SENT， But it is " + statusId,
						module);
				return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
						"CloudCardInvitationStatusUnexpected", locale));
			}
			...
			if (null != oldStoreGroup) {
				...
				Debug.logWarning(
						"This store [" + organizationPartyId + "] is already in a Group, Can Not join another one.",
						module);
				return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
						"CloudCardIsInGroupCanNotJoinAnother", locale));
			}
			...
			if (!isAccept) {
				...
				if (UtilValidate.isNotEmpty(partyInvitationRoleAssocList)) {
					Map<String, Object> tmpCtx = UtilMisc.toMap("locale", locale, "userLogin", systemUser, "partyId",
							organizationPartyId);
				...
				if (UtilValidate.isNotEmpty(partyInvitationGroupAssocList)) {
					Map<String, Object> tmpCtx = UtilMisc.toMap("locale", locale, "userLogin", systemUser, "partyIdTo",
							organizationPartyId, "roleTypeIdTo", CloudCardConstant.STORE_GROUP_PARTNER_ROLE_TYPE_ID,
							"partyRelationshipTypeId", CloudCardConstant.STORE_GROUP_PARTY_RELATION_SHIP_TYPE_ID,
							"roleTypeIdFrom", CloudCardConstant.STORE_GROUP_ROLE_TYPE_ID, "statusId",
					...
					for (GenericValue gv : partyInvitationGroupAssocList) {
						...
						Map<String, Object> createPartyRelationshipOut = dispatcher.runSync("createPartyRelationship",
								tmpCtx);
	...
	public static Map<String, Object> bizExitGroup(DispatchContext dctx, Map<String, Object> context) {
		...
		try {
			...
			GenericValue myPartyRelationship = CloudCardHelper.getGroupRelationShipByStoreId(delegator,
					organizationPartyId, false);
			...
			if (UtilValidate.isEmpty(myGroupId)) {
				...
				Debug.logWarning(
						"This store[" + organizationPartyId + "] is not in a group now! this operation will be ignored",
						module);
			...
			if (!UtilValidate.areEqual(storeId, organizationPartyId)) {
				...
				if (!isGroupOwner) {
					// 如果不是圈主，则不能踢人
					Debug.logWarning("The store[" + organizationPartyId + "] is not the owner of the group[" + myGroupId
							+ "], can't kick out this store[" + storeId + "]", module);
					return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
							"CloudCardNotGroupOwnerCanNotKickOthers", locale));
				}

				GenericValue groupStoreRelationship = CloudCardHelper.getGroupRelationShipByStoreId(delegator, storeId,
						false);
				if (UtilValidate.isEmpty(groupStoreRelationship)) {
					// 要踢出的店已经不在圈子里面了，忽略本次调用，直接返回成功
					Debug.logWarning(
							"This store[" + storeId + "] is not in a group now! this operation will be ignored",
							module);
					return ServiceUtil.returnSuccess();
				}
				...
				if (!myGroupId.equals(groupStoreRelationship.getString("partyIdFrom"))) {
					...
					Debug.logWarning("This store[" + storeId + "] is not in my group[" + myGroupId
							+ "! this operation will be ignored", module);
					return ServiceUtil.returnSuccess();
				}

				// 对面也是圈主，不能踢出 * 业务上应该不存在这样的情况
				if (CloudCardHelper.isStoreGroupOwnerRelationship(groupStoreRelationship)) {
					Debug.logWarning("This store[" + storeId + "] is also the owner of group[" + myGroupId
							+ "], can't kick it out", module);
					return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
							"CloudCardCanNotKickOutGroupOwner", locale));
				}
				...
				if (!CloudCardConstant.SG_REL_STATUS_FROZEN.equalsIgnoreCase(relStatusId)) {
					Debug.logWarning("Must first freeze this store[" + storeId + "] of the group[" + myGroupId
							+ "], to kick out " + relStatusId, module);
					return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
							"CloudCardMustFirstFreezeToKickOut", locale));
				}
				...
				if (settlementAmount.compareTo(CloudCardHelper.ZERO) != 0) {
					...
					return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
							"CloudCardTheStoreNotSettled", locale));
				}
				...
				if (isGroupOwner) {
					...
					List<String> partnerIdList = CloudCardHelper.getStoreGroupPartnerIdListByGroupId(delegator,
							myGroupId, true);
					if (null != partnerIdList && partnerIdList.size() > 1) {
						Debug.logWarning("There are other partners in the Group[" + myGroupId + "], the owner["
								+ organizationPartyId + "] can not quit group", module);
						return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
								"CloudCardPartnerExistsCanNotExit", locale));
					}
				}
				// 判断是否结清跨店交易产生的 未结算金额
				BigDecimal settlementAmount = CloudCardHelper.getSettlementAmountByStoreId(delegator,
						organizationPartyId);
				if (settlementAmount.compareTo(CloudCardHelper.ZERO) != 0) {
					Debug.logWarning("店铺[" + organizationPartyId + "] 未结算金额:" + settlementAmount.toPlainString(),
							module);
					return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
							"CloudCardTheStoreNotSettled", locale));
				}
	...
	public static Map<String, Object> bizFreezeGroupPartner(DispatchContext dctx, Map<String, Object> context) {
		...
		try {
			...
			if (!isGroupOwner) {
				// 不是圈主，没权冻结别人，直接返回失败
				Debug.logWarning("This store[" + organizationPartyId
						+ "] is not the owner of group, can not freeze/unfreeze the partner", module);
				return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
						"CloudCardNotGroupOwnerCanNotFreezeOthers", locale));
			}

			// 获取要冻结的店的 圈子关系
			GenericValue storeGroupRelationship = CloudCardHelper.getGroupRelationShipByStoreId(delegator, storeId,
					false);
			if (null == storeGroupRelationship) {
				// 店铺不在圈子里，不能冻结、解冻
				Debug.logWarning("Store[" + storeId + "] is not in the group, can't freeze/unfreeze it", module);
				return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
						"CloudCardStoreNotInAGroup", locale));
			}
			...
			if (!UtilValidate.areEqual(myGroupId, storeGroupId)) {
				// 要冻结的店铺与自己店铺不在同一个圈子中
				Debug.logWarning("Store[" + storeId + "] is not in the same group with my store[" + organizationPartyId
						+ "], can't freeze/unfreeze it", module);
				return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
						"CloudCardNotInTheSameGroup", locale));
			}

			if (CloudCardHelper.isStoreGroupOwnerRelationship(storeGroupRelationship)) {
				// 被冻结或解冻的店铺也是一个圈主，返回失败
				Debug.logWarning("This store[" + storeId + "] is also the owner of group, can't freeze/unfreeze it",
						module);
				return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
						"CloudCardCanNotFreezeGroupOwner", locale));
			}
	...
	public static Map<String, Object> bizDissolveGroup(DispatchContext dctx, Map<String, Object> context) {
		...
		try {
			...
			if (null == myGroupRelationship) {
				// 如果不存在圈子，直接返回成功
				Debug.logWarning("This store[" + organizationPartyId + "] is not in a group now! just return success.",
						module);
				return ServiceUtil.returnSuccess();
			}
			isGroupOwner = CloudCardHelper.isStoreGroupOwnerRelationship(myGroupRelationship);
			...
			if (!isGroupOwner) {
				// 不是圈主，没权解散圈子，返回失败
				Debug.logWarning(
						"This store[" + organizationPartyId + "] is not the owner of group, can not dissolve the group",
						module);
				return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
						"CloudCardNotGroupOwnerCanNotDissolveGroup", locale));
			}

			String myGroupId = CloudCardHelper.getGroupIdByRelationship(myGroupRelationship);

			List<GenericValue> storeRelList = CloudCardHelper.getStoreGroupRelationshipByGroupId(delegator, myGroupId,
					false);
			...
			if (UtilValidate.isNotEmpty(onlyPartnerRelList)) {
				...
				for (GenericValue gv : onlyPartnerRelList) {
					if (!CloudCardHelper.isFrozenGroupRelationship(gv)) {
						// 有没有冻结的，不能解散
						Debug.logWarning("Store[" + gv.getString("partyIdTo")
								+ "] is Not frozen store in a group,can not dissolve the group! ", module);
						return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
								"CloudCardMustFreezeAllToDissolve", locale));
					}
				}
			}
			...
			Map<String, Object> setPartyStatusOut = dispatcher.runSync("setPartyStatus", UtilMisc.toMap("userLogin",
					systemUserLogin, "locale", locale, "statusId", "PARTY_DISABLED", "partyId", myGroupId));
			...
			try {
				existPartyInvitations = delegator.findList("PartyInvitation",
						EntityCondition.makeCondition(
								UtilMisc.toMap("partyId", organizationPartyId, "statusId", "PARTYINV_SENT")),
						null, UtilMisc.toList("-" + ModelEntity.CREATE_STAMP_FIELD), null, false);
				if (UtilValidate.isNotEmpty(existPartyInvitations)) {
					...
					for (GenericValue gv : existPartyInvitations) {
						...
						// 验证是否为 邀请加入圈子，
						GenericValue partyInvitationRoleAssoc = delegator.findByPrimaryKeyCache(
								"PartyInvitationRoleAssoc", UtilMisc.toMap("partyInvitationId", partyInvitationId,
										"roleTypeId", CloudCardConstant.STORE_GROUP_PARTNER_ROLE_TYPE_ID));
						if (null == partyInvitationRoleAssoc) {
							continue;
						}

						GenericValue partyFrom = delegator.findByPrimaryKeyCache("PartyGroup",
		...
		try {
			...
			if (UtilValidate.isNotEmpty(groupMemberRelationsList)) {
				...
				for (GenericValue gv : groupMemberRelationsList) {
					...
					GenericValue store = delegator.findByPrimaryKeyCache("PartyGroup",
							UtilMisc.toMap("partyId", storeId));
					boolean tmpIsOwner = CloudCardHelper.isStoreGroupOwnerRelationship(gv);
					Map<String, String> tmpMap = UtilMisc.toMap("storeId", storeId, "storeName",
							store.getString("groupName"));
					...
					tmpMap.put("isFrozen", CloudCardHelper.isFrozenGroupRelationship(gv) ? CloudCardConstant.IS_Y
							: CloudCardConstant.IS_N);
					if (tmpIsOwner) {
	...
	public static Map<String, Object> bizDoSettlement(DispatchContext dctx, Map<String, Object> context) {
		...
		if (settlementAmount.compareTo(CloudCardHelper.ZERO) <= 0
				|| actualSettlementAmount.compareTo(CloudCardHelper.ZERO) <= 0) {
			Debug.logWarning("金额不合法，必须为正数", module);
			return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceAccountingError,
		...
		if (storeAmount.compareTo(CloudCardHelper.ZERO) == 0) {
			...
			return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
					"CloudCardTheStoreHasBeenSettled", locale));
		}
		...
		try {
			...
			// 检查是否已有发出但对方还未处理的结算
			List<GenericValue> oldPayments = delegator.findByAnd("Payment",
					UtilMisc.toMap("partyIdFrom", organizationPartyId, "partyIdTo", storeId, "statusId", "PMNT_SENT",
							"paymentTypeId", "DISBURSEMENT", "paymentMethodTypeId", CloudCardConstant.PMT_CASH,
			...
			if (UtilValidate.isNotEmpty(oldPayments)) {
				...
				return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
						"CloudCardTheStoreRequestHasBeenIssued", locale));
			}

			// 扣除本店结算账户相应的金额
			finAccountWithdrawalOutMap = dispatcher.runSync("createPaymentAndFinAccountTransForCloudCard",
					UtilMisc.toMap("userLogin", systemUserLogin, "locale", locale, "statusId", "PMNT_SENT",
							"currencyUomId", CloudCardConstant.DEFAULT_CURRENCY_UOM_ID, "actualCurrencyUomId",
							CloudCardConstant.DEFAULT_CURRENCY_UOM_ID, "finAccountTransTypeId", "WITHDRAWAL",
							"paymentTypeId", "DISBURSEMENT", "finAccountId", myAccount.getString("finAccountId"),
							"paymentMethodTypeId", CloudCardConstant.PMT_CASH, "partyIdFrom", organizationPartyId,
							"partyIdTo", storeId, "roleTypeIdTo", CloudCardConstant.STORE_GROUP_PARTNER_ROLE_TYPE_ID,
							"amount", settlementAmount, "actualCurrencyAmount", actualSettlementAmount, "comments",
							"结算：圈主付款", "reasonEnumId", "FATR_PURCHASE"));
		} catch (GenericServiceException | GenericEntityException e1) {
			Debug.logError(e1.getMessage(), module);
			return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
					"CloudCardInternalServiceError", locale));
	...
	public static Map<String, Object> bizSettlementConfirm(DispatchContext dctx, Map<String, Object> context) {
		...
		try {
			...
			// 检查结算的状态
			GenericValue settlePayment = delegator.findByPrimaryKey("Payment",
					UtilMisc.toMap("paymentId", settlementId));
			if (UtilValidate.isEmpty(settlePayment)) {
				Debug.logWarning("无效的结算", module);
				return ServiceUtil.returnError(
						UtilProperties.getMessage(CloudCardConstant.resourceError, "CloudCardParamIllegal", locale));
			}
			...
			if (!"PMNT_SENT".equals(statusId)) {
				...
				return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
						"CloudCardSettlementStatusUnexpected", locale));
			}
			...
			if (CloudCardConstant.IS_N.equals(isConfirm)) {
				Map<String, Object> setPaymentStatusOutMap = dispatcher.runSync("setPaymentStatus",
						UtilMisc.toMap("userLogin", systemUserLogin, "locale", locale, "paymentId", settlementId,
			...
			Map<String, Object> finAccountReceiptOutMap = dispatcher.runSync(
					"createPaymentAndFinAccountTransForCloudCard",
					UtilMisc.toMap("userLogin", systemUserLogin, "locale", locale, "statusId", "PMNT_CONFIRMED",
							"currencyUomId", CloudCardConstant.DEFAULT_CURRENCY_UOM_ID, "actualCurrencyUomId",
							CloudCardConstant.DEFAULT_CURRENCY_UOM_ID, "finAccountTransTypeId", "DEPOSIT",
							"paymentTypeId", "RECEIPT", "finAccountId", myAccount.getString("finAccountId"),
							"paymentMethodTypeId", CloudCardConstant.PMT_CASH, "partyIdFrom", partyIdFrom, "partyIdTo",
							organizationPartyId, "roleTypeIdTo", CloudCardConstant.STORE_GROUP_PARTNER_ROLE_TYPE_ID,
							"amount", settlementAmount, "actualCurrencyAmount", actualSettlementAmount, "comments",
							"结算：圈友收款", "reasonEnumId", "FATR_PURCHASE"));
			...
			Map<String, Object> paymentApplicationOutMap = dispatcher.runSync("createPaymentApplication",
					UtilMisc.toMap("userLogin", systemUserLogin, "locale", locale, "amountApplied", settlementAmount,
							"paymentId", settlementId, "toPaymentId", receiptPaymentId));
	 ...
	 * @deprecated
	 */
	public static Map<String, Object> bizGetUnconfirmedSettlementInfo(DispatchContext dctx,
		...
		try {
			...
			// 检查是否已有发出但对方还未处理的结算
			GenericValue settlement = EntityUtil.getFirst(delegator.findByAnd("Payment",
					UtilMisc.toMap("partyIdFrom", groupOwnerId, "partyIdTo", storeId, "statusId", "PMNT_SENT",
							"paymentTypeId", "DISBURSEMENT", "paymentMethodTypeId", CloudCardConstant.PMT_CASH,
							"roleTypeIdTo", CloudCardConstant.STORE_GROUP_PARTNER_ROLE_TYPE_ID),
					null));
			if (null != settlement) {
	...
	private static Map<String, Object> checkInputParam(DispatchContext dctx, Map<String, Object> context) {
		...
		if (UtilValidate.isNotEmpty(organizationPartyId)) {
			...
			try {
				organization = delegator.findByPrimaryKeyCache("PartyGroup",
			...
			if (null == organization) {
				...
				return ServiceUtil.returnError(
						UtilProperties.getMessage(CloudCardConstant.resourceError, "CloudCardOrganizationPartyNotFound",
			...
			if (!CloudCardHelper.isManager(delegator, userLogin.getString("partyId"), organizationPartyId)) {
				Debug.logError(
						"partyId: " + userLogin.getString("partyId") + " 不是商户：" + organizationPartyId + "的管理人员，不能操作",
						module);
				return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
	...
	public static Map<String, Object> getCloudcardsOfUser(DispatchContext dctx, Map<String, Object> context) {
		...
		if (UtilValidate.isNotEmpty(qrCode) && qrCode.startsWith(CloudCardConstant.CODE_PREFIX_PAY_)) {
			...
			try {
				...
				return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
						"CloudCardPaymentCodeExpired", locale));
			} catch (Exception e) {
				// 非法的码
				Debug.logError(e.getMessage(), module);
				return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
						"CloudCardPaymentCodeIllegal", locale));
			}
			...
			if (UtilValidate.isEmpty(customer)) {
				return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
						"CloudCardUserNotExistError", locale));
			}
	...
	public static Map<String, Object> activateCloudCardAndRechargeByTelNumber(DispatchContext dctx,
			Map<String, Object> context) {
		LocalDispatcher dispatcher = dctx.getDispatcher();
		Delegator delegator = dctx.getDelegator();
		Locale locale = (Locale) context.get("locale");
		...
		EntityCondition captchaCondition = EntityCondition.makeCondition(
				...
				EntityUtil.getFilterByDateExpr(), EntityCondition.makeCondition("isValid", EntityOperator.EQUALS, "N"),
				EntityCondition.makeCondition("smsType", EntityOperator.EQUALS,
						CloudCardConstant.USER_PURCHASE_CARD_CAPTCHA_SMS_TYPE));
		...
		try {
			activateCloudCardAndRechargeOut = dispatcher.runSync("activateCloudCardAndRecharge",
					UtilMisc.toMap("userLogin", userLogin, "organizationPartyId", organizationPartyId, "teleNumber",
							teleNumber, "amount", amount));
		} catch (GenericServiceException e) {
			Debug.logError(e.getMessage(), module);
			return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
					"CloudCardInternalServiceError", locale));
		...
		if (activateCloudCardAndRechargeOut.containsKey("errorMessage")) {
			if ("Users have cards in our store"
					.equalsIgnoreCase(activateCloudCardAndRechargeOut.get("errorMessage").toString())) {
				return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
						"CloudCardUsersHaveCardsInOurStore", locale));
			}
		}
	...
	public static Map<String, Object> bizByTeleNumberWithdraw(DispatchContext dctx, Map<String, Object> context) {
		...
		EntityCondition captchaCondition = EntityCondition.makeCondition(
				...
				EntityUtil.getFilterByDateExpr(), EntityCondition.makeCondition("isValid", EntityOperator.EQUALS, "N"),
				EntityCondition.makeCondition("smsType", EntityOperator.EQUALS,
						CloudCardConstant.USER_PAY_CAPTCHA_SMS_TYPE));
		...
		// 数据权限检查，先放这里
		if (!CloudCardHelper.isManager(delegator, userLogin.getString("partyId"), organizationPartyId)) {
			Debug.logWarning("partyId: " + userLogin.getString("partyId") + " 不是商户：" + organizationPartyId
					+ "的管理人员，不能对用户卡进行扫码消费", module);
			return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
					"CloudCardUserLoginIsNotManager", locale));
		}
		...
		if (UtilValidate.isEmpty(customer) || UtilValidate.isEmpty(cloudCards)) {
			try {
				...
				if(UtilValidate.isNotEmpty(createInfoOutMap)){
					...
					cardBalance = new BigDecimal(createInfoOutMap.get("cardBalance").toString()) ;
				}
			...
			if (UtilValidate.isEmpty(cloudcardList)) {
				return ServiceUtil.returnError(
						UtilProperties.getMessage(CloudCardConstant.resourceError, "CloudCardNoCardInTheStore", locale));
			}
			...
			try {
				rechargeCloudCardOutMap = dispatcher.runSync("rechargeCloudCard",
						UtilMisc.toMap("userLogin", userLogin, "locale", locale, "organizationPartyId", organizationPartyId,
								"cardId", cloudCardMap.get("cardId"), "amount", amount));
			} catch (GenericServiceException e) {
				Debug.logError(e.getMessage(), module);
				return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
						"CloudCardInternalServiceError", locale));
	...
	public static Map<String, Object> listMyNote(DispatchContext dctx, Map<String, Object> context) {
		...
		try {
			partyNotes = delegator.findList("PartyNoteView2",
					EntityCondition.makeCondition(UtilMisc.toMap("partyId", organizationPartyId, "isViewed", "N")),
					null, UtilMisc.toList("-noteDateTime"), null, false);
		} catch (GenericEntityException e) {
			Debug.logError(e.getMessage(), module);
			return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
					"CloudCardInternalServiceError", locale));
	...
	public static Map<String, Object> listMyHistoryNote(DispatchContext dctx, Map<String, Object> context) {
		...
		try {
			partyNotes = delegator.findList("PartyNoteView2",
					EntityCondition.makeCondition(UtilMisc.toMap("partyId", organizationPartyId, "removed", "N")), null,
					UtilMisc.toList("-noteDateTime"), null, false);
		} catch (GenericEntityException e) {
			Debug.logError(e.getMessage(), module);
			return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
					"CloudCardInternalServiceError", locale));
	...
	public static Map<String, Object> readMyNote(DispatchContext dctx, Map<String, Object> context) {
		...
		try {
			partyNote = delegator.findByPrimaryKey("PartyNote",
					UtilMisc.toMap("partyId", organizationPartyId, "noteId", noteId));
		...
		try {
			listMyNoteMap = dispatcher.runSync("listMyNote",
					UtilMisc.toMap("organizationPartyId", organizationPartyId, "userLogin", userLogin));
		} catch (GenericServiceException e) {
			Debug.logError(e.getMessage(), module);
			return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
					"CloudCardInternalServiceError", locale));
	...
	public static Map<String, Object> bizDeleteStoreInfoImg(DispatchContext dctx, Map<String, Object> context) {
		...
		try {
			List<GenericValue> partyContents = delegator.findByAnd("PartyContent",
					UtilMisc.toMap("contentId", contentId, "partyId", organizationPartyId));
			if (UtilValidate.isEmpty(partyContents)) {
				return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
						"CloudCardPictureDoesNotExist", locale));
			}
        ...
        try {
        	bizLicImgList = delegator.findByAnd("PartyContentAndDataResourceDetail", UtilMisc.toMap("partyId", organizationPartyId,"partyContentTypeId", "STORE_IMG","contentTypeId","ACTIVITY_PICTURE","statusId","CTNT_IN_PROGRESS", "dataResourceName","bizLic"));
		} catch (GenericEntityException e) {
			Debug.logError(e.getMessage(), module);
            return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError, "CloudCardInternalServiceError", locale));
		}
        ...
        List<GenericValue> bizAvatarImgList = FastList.newInstance();
        try {
        	bizAvatarImgList = delegator.findByAnd("PartyContentAndDataResourceDetail", UtilMisc.toMap("partyId", organizationPartyId,"partyContentTypeId", "STORE_IMG","contentTypeId","ACTIVITY_PICTURE","statusId","CTNT_IN_PROGRESS", "dataResourceName","bizAvatar"));
		} catch (GenericEntityException e) {
			Debug.logError(e.getMessage(), module);
            return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError, "CloudCardInternalServiceError", locale));
		}
        ...
        try {
        	bizDetailsList = delegator.findByAnd("PartyContentAndDataResourceDetail", UtilMisc.toMap("partyId", organizationPartyId,"partyContentTypeId", "STORE_IMG","contentTypeId","ACTIVITY_PICTURE","statusId","CTNT_IN_PROGRESS", "dataResourceName","bizDetails"));
		} catch (GenericEntityException e) {
			Debug.logError(e.getMessage(), module);
            return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError, "CloudCardInternalServiceError", locale));
		}
		...
		// 获取oss访问地址
		String ossUrl = EntityUtilProperties.getPropertyValue("cloudcard", "oss.url",
				"http://kupang.oss-cn-shanghai.aliyuncs.com/", delegator);
		Map<String, Object> result = ServiceUtil.returnSuccess();
	...
	public static Map<String, Object> getCloudCardByBiz(DispatchContext dctx, Map<String, Object> context) {
		...
		if (!CloudCardHelper.isManager(delegator, userLogin.getString("partyId"), organizationPartyId)) {
			...
			Debug.logError("partyId: " + userLogin.getString("partyId") + " 不是商户：" + organizationPartyId
					+ "的管理人员，不能进行账户流水查询操作", module);
			return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
					"CloudCardUserLoginIsNotManager", locale));
		}
		...
		try {
			// 有可能存在授权的卡
			EntityCondition thruDateEntityCondition = EntityCondition.makeCondition(EntityOperator.OR,
			...
			EntityCondition cloudCardInfoEntityCondition = EntityCondition.makeCondition(EntityOperator.AND,
					EntityCondition.makeCondition(UtilMisc.toMap("distributorPartyId", organizationPartyId)),
					thruDateEntityCondition);
			...
			for (GenericValue finAccount : finAccountList) {
				...
				List<GenericValue> partyAndTelecomNumbers = delegator.findByAndCache("PartyAndTelecomNumber",
						UtilMisc.toMap("partyId", partyId, "partyTypeId", "PERSON"));
	...
	public static Map<String, Object> getUserPaymentBybiz(DispatchContext dctx, Map<String, Object> context) {
		...
		try {
			GenericValue encryptedGiftCard = delegator.makeValue("FinAccount",
					UtilMisc.toMap("finAccountCode", cardNumber));
			delegator.encryptFields(encryptedGiftCard);
			finAccountAndRoles = delegator.findList("CloudCardInfo",
					EntityCondition.makeCondition(UtilMisc.toMap("distributorPartyId", organizationPartyId,
							"finAccountCode", encryptedGiftCard.getString("finAccountCode"))),
					UtilMisc.toSet("finAccountCode", "finAccountId", "ownerPartyId"), null, null, true);
		} catch (GenericEntityException e1) {
			Debug.logError(e1.getMessage(), module);
			return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
					"CloudCardInternalServiceError", locale));
		...
		if (UtilValidate.isEmpty(finAccountAndRoles)) {
			return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
					"CloudCardNotOurCardCanNotBeQuery", locale));
		}

		/*
		 * Timestamp fromDate =(Timestamp)context.get("fromTime"); fromDate =
		 * UtilDateTime.getMonthStart(fromDate,0);
		 *
		 * Timestamp thruDate =(Timestamp)context.get("thruTime"); thruDate =
		 * UtilDateTime.getDayStart(thruDate, 1);
		 *
		 * EntityCondition timeConditions =
		 * EntityCondition.makeCondition("effectiveDate",
		 * EntityOperator.BETWEEN, UtilMisc.toList(fromDate, thruDate));
		 */

		EntityCondition paymentConditions = null;
		if ("1".equals(type)) {
			/*
			 * EntityCondition depositConditions =
			 * EntityCondition.makeCondition(UtilMisc.toMap("paymentTypeId",
			 * "GC_DEPOSIT", "partyIdTo", partyId)); paymentConditions =
			 * EntityCondition.makeCondition(EntityOperator.AND,
			 * depositConditions, timeConditions);
			 */

			paymentConditions = EntityCondition
					.makeCondition(UtilMisc.toMap("paymentTypeId", "GC_DEPOSIT", "partyIdTo", partyId));

		} else if ("2".equals(type)) {
			/*
			 * EntityCondition withDrawalCondition =
			 * EntityCondition.makeCondition(UtilMisc.toMap("paymentTypeId",
			 * "GC_WITHDRAWAL", "partyIdFrom", partyId)); paymentConditions =
			 * EntityCondition.makeCondition(EntityOperator.AND,
			 * withDrawalCondition, timeConditions);
			 */

			paymentConditions = EntityCondition
					.makeCondition(UtilMisc.toMap("paymentTypeId", "GC_WITHDRAWAL", "partyIdFrom", partyId));

		} else {
			/*
			 * EntityCondition depositCond =
			 * EntityCondition.makeCondition(UtilMisc.toMap("paymentTypeId",
			 * "GC_DEPOSIT", "partyIdTo", partyId)); EntityCondition
			 * withDrawalCond =
			 * EntityCondition.makeCondition(UtilMisc.toMap("paymentTypeId",
			 * "GC_WITHDRAWAL", "partyIdFrom", partyId)); EntityCondition
			 * allConditions = EntityCondition.makeCondition(EntityOperator.OR,
			 * depositCond, withDrawalCond); paymentConditions =
			 * EntityCondition.makeCondition(EntityOperator.AND, allConditions,
			 * timeConditions);
			 */

			EntityCondition depositCond = EntityCondition
					.makeCondition(UtilMisc.toMap("paymentTypeId", "GC_DEPOSIT", "partyIdTo", partyId));
			EntityCondition withDrawalCond = EntityCondition
					.makeCondition(UtilMisc.toMap("paymentTypeId", "GC_WITHDRAWAL", "partyIdFrom", partyId));
		...
		if (UtilValidate.isNotEmpty(cardId)) {
			paymentConditions = EntityCondition.makeCondition(paymentConditions,
					EntityCondition.makeCondition("paymentMethodId", cardId));
		}
		...
		try {
			eli = delegator.find(
					"PaymentAndTypePartyNameView", paymentConditions, null, UtilMisc.toSet("amount",
							"partyFromGroupName", "partyToGroupName", "paymentTypeId", "effectiveDate"),
					UtilMisc.toList("-effectiveDate"), null);
		} catch (GenericEntityException e) {
			Debug.logError(e.getMessage(), module);
			return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
					"CloudCardInternalServiceError", locale));
		...
		for (GenericValue payment : payments) {
			...
			paymentMap.put("transDate",
					UtilDateTime.toCalendar(payment.getTimestamp("effectiveDate")).getTimeInMillis());
	...
	public static Map<String, Object> initiateSettlement(DispatchContext dctx, Map<String, Object> context) {
		...
		if (!CloudCardHelper.isManager(delegator, userLogin.getString("partyId"), payeePartyId)) {
			// 若不是 system userLogin，则需要验证是否是本店的manager
			Debug.logError(
					"partyId: " + userLogin.getString("partyId") + " 不是商户：" + payeePartyId + "的管理人员，不能进行账户流水查询操作",
					module);
			return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
					"CloudCardUserLoginIsNotManager", locale));
		}
		...
		try {
			...
			// 极光推送消息
			dispatcher.runSync("pushNotifOrMessage", UtilMisc.toMap("userLogin", userLogin, "appType", "biz", "content",
					noteInfo, "title", "发起结算请求", "sendType", "tag", "tag", payerPartyId));
			// 系统记录消息
			dispatcher.runSync("saveMyNote", UtilMisc.toMap("partyId", payerPartyId, "paymentId", paymentId, "noteName",
					"INITIATE_SETTLEMENT", "noteInfo", noteInfo));
		} catch (GenericServiceException e) {
			Debug.logError(e.getMessage(), module);
			return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
					"CloudCardInternalServiceError", locale));
	...
	public static Map<String, Object> settlementRequest(DispatchContext dctx, Map<String, Object> context) {
		...
		if (!CloudCardHelper.isManager(delegator, userLogin.getString("partyId"), payerPartyId)) {
			// 若不是 system userLogin，则需要验证是否是本店的manager
			Debug.logError(
					"partyId: " + userLogin.getString("partyId") + " 不是商户：" + payerPartyId + "的管理人员，不能进行账户流水查询操作",
					module);
			return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
					"CloudCardUserLoginIsNotManager", locale));
		}
		...
		EntityCondition sentCond = EntityCondition
				.makeCondition(UtilMisc.toMap("statusId", "PMNT_SENT", "paymentId", paymentId));
		try {
		...
		try {
			...
			// 极光推送消息
			dispatcher.runSync("pushNotifOrMessage", UtilMisc.toMap("userLogin", userLogin, "appType", "biz", "content",
					noteInfo, "title", "发起结算请求", "sendType", "tag", "tag", payeePartyId));
			// 系统记录消息
			dispatcher.runSync("saveMyNote", UtilMisc.toMap("partyId", payeePartyId, "paymentId", paymentId, "noteName",
					"INITIATE_SETTLEMENT", "noteInfo", noteInfo));
		} catch (GenericServiceException e) {
			Debug.logError(e.getMessage(), module);
			return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
	...
	public static Map<String, Object> settlementConfirmation(DispatchContext dctx, Map<String, Object> context) {
		...
		if (UtilValidate.isEmpty(creditLimitAccount)) {
			...
			return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
					"CloudCardConfigError", UtilMisc.toMap("organizationPartyId", payerPartyId), locale));
		}
		...
		try {
			...
			if (!"PMNT_P_NOT_SENT_RCV".equals(payment.getString("statusId"))) {
				return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
						"CloudCardUnableToConfirmSettlement", locale));
			}
			...
			createFinAccountAuthOutMap = dispatcher.runSync("createFinAccountAuth", UtilMisc.toMap("userLogin",
					systemUserLogin, "locale", locale, "currencyUomId", CloudCardConstant.DEFAULT_CURRENCY_UOM_ID,
		...
		try {
			setPaymentStatusOutMap = dispatcher.runSync("setPaymentStatus", UtilMisc.toMap("userLogin", systemUserLogin,
					"locale", locale, "paymentId", paymentId, "statusId", "PMNT_CONFIRMED"));
		} catch (GenericServiceException e1) {
		...
		try {
			...
			// 极光推送消息
			dispatcher.runSync("pushNotifOrMessage", UtilMisc.toMap("userLogin", userLogin, "appType", "biz", "content",
					noteInfo, "title", "已确认结算", "sendType", "tag", "tag", payerPartyId));
			// 系统记录消息
			dispatcher.runSync("saveMyNote", UtilMisc.toMap("partyId", payeePartyId, "paymentId", paymentId, "noteName",
					"INITIATE_SETTLEMENT", "noteInfo", noteInfo));
		} catch (GenericServiceException e) {
		...
		try {
			settlementMap = dispatcher.runSync("bizListNeedSettlement",
					UtilMisc.toMap("userLogin", userLogin, "organizationPartyId", payeePartyId, "role", "payee"));
		} catch (GenericServiceException e) {
			Debug.logError(e.getMessage(), module);
			return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
					"CloudCardInternalServiceError", locale));
	 ...
	 * @return
	 */
	public static Map<String, Object> bizSetCloudcardSettlementPeriod(DispatchContext dctx,
			Map<String, Object> context) {
		LocalDispatcher dispatcher = dctx.getDispatcher();
		Delegator delegator = dispatcher.getDelegator();
		Locale locale = (Locale) context.get("locale");
		...
		if (!CloudCardHelper.isManager(delegator, userLogin.getString("partyId"), partyIdFrom)) {
			// 若不是 system userLogin，则需要验证是否是本店的manager
			Debug.logError("partyId: " + userLogin.getString("partyId") + " 不是商户：" + partyIdFrom + "的管理人员，不能进行账户流水查询操作",
					module);
			return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
					"CloudCardUserLoginIsNotManager", locale));
		}
		...
		try {
			partyIdFromSettlementPeriod = delegator.findOne("CloudcardSettlementPeriod", false,
					UtilMisc.toMap("partyIdFrom", partyIdFrom, "partyIdTo", partyIdTo));
			partyIdToSettlementPeriod = delegator.findOne("CloudcardSettlementPeriod", false,
					UtilMisc.toMap("partyIdFrom", partyIdTo, "partyIdTo", partyIdFrom));
		} catch (GenericEntityException e) {
			Debug.logError(e.getMessage(), module);
			return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
					"CloudCardInternalServiceError", locale));
		...
		if (UtilValidate.isEmpty(partyIdFromSettlementPeriod) && UtilValidate.isEmpty(partyIdToSettlementPeriod)) {
			try {
				GenericValue cloudcardSettlementPeriod = delegator.makeValue("CloudcardSettlementPeriod",
						UtilMisc.toMap("partyIdFrom", partyIdFrom, "partyIdTo", partyIdTo, "status", "N", "week", week,
								"hour", hour));
	 ...
	 * @return
	 */
	public static Map<String, Object> bizGetCloudcardSettlementPeriod(DispatchContext dctx,
			Map<String, Object> context) {
		LocalDispatcher dispatcher = dctx.getDispatcher();
		Delegator delegator = dispatcher.getDelegator();
		Locale locale = (Locale) context.get("locale");
		...
		GenericValue partyIdToSettlementPeriod = null;
		try {
			partyIdFromSettlementPeriod = delegator.findOne("CloudcardSettlementPeriod", true,
					UtilMisc.toMap("partyIdFrom", partyIdFrom, "partyIdTo", partyIdTo));
			partyIdToSettlementPeriod = delegator.findOne("CloudcardSettlementPeriod", true,
					UtilMisc.toMap("partyIdFrom", partyIdTo, "partyIdTo", partyIdFrom));
		} catch (GenericEntityException e) {
			Debug.logError(e.getMessage(), module);
			return ServiceUtil.returnError(UtilProperties.getMessage(CloudCardConstant.resourceError,
